...
Deno.serve(async (req) => {
  ...
  if (action === "email") {
    ...
    const prompt = [
      ...
      `Hi,`,
      ``,
      `I came across ${name} while researching ${industry} companies and noticed you've built a solid reputation with your customers.${goodRating ? ` A ${rating!.toFixed(1)}-star rating on Google is something most businesses would love to have.` : ``}`,
      ``,
      ...
      ``,
      `Nothing major—just a handful of improvements that could help convert more of the traffic you're already getting. Google has made some really cool improvements to Business Profiles that make it way easier for customers to reach out right from your listing.`,
      ``,
      `I put together a quick analysis and would be happy to share it with you. If you'd like me to send over the findings, just let me know. If you want to check out what I do beforehand, you can go to www.elipsiongroup.com`,
      ``,
      `Thanks,`,
      `${senderName}`,
      `---`,
      ...
      `- Mention, warmly and casually, that Google has made recent improvements to Business Profiles that make it easier for customers to reach out right from the listing.`,
      `- Offer to share a quick analysis / the findings.`,
      `- End with a soft, no-pressure ask like "if you'd like me to send over the findings, just let me know," followed by an offer to check out the work beforehand at www.elipsiongroup.com.`,
      `- Under ~180 words. Plain text. Sign off simply as "${senderName}" — do NOT add a company name line after it.`,
